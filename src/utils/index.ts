...
import { JSONArray, JSONObject, Result, Unpacked } from "@/types/index";
...
export function loop_request<
  T extends {
    fetch: (...args: any[]) => Promise<any>;
    can_finish: (r: Unpacked<ReturnType<T["fetch"]>>) => Promise<boolean>;
    handler: (r: Unpacked<ReturnType<T["fetch"]>>) => void;
  }
>(options: T): (...args: Parameters<T["fetch"]>) => Promise<Unpacked<ReturnType<T["fetch"]>>> {
  const { fetch, can_finish, handler } = options;
  const request = fetch;
  return (...args: Parameters<T["fetch"]>) => {
    return new Promise((resolve) => {
      async function run() {
