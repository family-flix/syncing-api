...
import * as wc from '../crypto';

import type { Factory, JWT, Options } from '../jwt.d';
...
export function encode(input: object) {
	return Base64.base64url(buff.toBinary(buff.asUTF8(JSON.stringify(input))));
}
...
function toJSON<T>(input: string): T {
	return JSON.parse(buff.toUTF8(buff.asBinary(input)));
}

// type: external
export function decode<P, H>(
	input: string
): {
	header: JWT.Header<H>;
...
export function RSA<P, H>(bits: SIZE, options: Options.RSA<H>): Factory<P, H> {
	...
	return {
		async sign(payload) {
			let key = await crypto.subtle.importKey('pkcs8', buff.asPEM(privkey), keyer, false, ['sign']);
		...
		async verify(input) {
			...
			let load = hh + '.' + pp;
			let sign = buff.asBinary(Base64.decode(ss));

			let key = await crypto.subtle.importKey('spki', buff.asPEM(pubkey), keyer, false, ['verify']);
