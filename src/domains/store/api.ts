  ...
  prisma: PrismaClient = {
    ...
    user: {
      ...
      findFirst: async (params: { where: { id: string | number }; include?: { settings?: boolean } }) => {
        ...
        const r = await this.client.post(`/api/v1/user/find_first`, args);
        if (r.error) {
    ...
    drive: {
      ...
      findFirst: async (params: {
        ...
        const r = await this.client.post(`/api/v1/drive/find_first`, {
          where: { id, unique_id },
          include: { drive_token },
        });
        if (r.error) {
      ...
      update: async (params: {
        ...
        const r = await this.client.post(`/api/v1/drive/update`, {
          where: { id, unique_id },
          data: { drive_token },
        });
        if (r.error) {
    ...
    drive_token: {
      ...
      findFirst: async (params: { where: { id: string | number }; include?: { drive_token?: boolean } }) => {
        const { where, include = {} } = params;
        const { id } = where;
        const r = await this.client.post(`/api/v1/drive_token/find_first`, {
          where: { id },
        });
        if (r.error) {
          throw new Error(r.error.message);
      ...
      update: async (params: { where: { id: string | number }; data?: { data: string; expired_at: string } }) => {
        const { where, data = {} } = params;
        const { id } = where;
        const r = await this.client.post(`/api/v1/drive_token/update`, {
          where: { id },
          data,
        });
        if (r.error) {
